...
// Storing All Variables As Global Variables, I HAve Performed A Similar Task In Lab Thats why and its difficult to deal with parameters in this type of code
// GAME DATA
int totalGames = 20;
int gameCount = 20;
int gameSold[50] = {0};
...
// USER LIBRARY + HISTORY OF PURCHASINg
int library[50];
int libraryCount = 0;
int history[5];
int historyCount = 0;

// ALl FUntctions (Function Declaration)

void showHeader(); // For Header
...
void userMenu()
{
    while (true)
    {
        ...
        if      (userAccountOption == "1") userViewGames();
        else if (userAccountOption == "2") userBuyGame();
        else if (userAccountOption == "3") userRefundGame();
        else if (userAccountOption == "4") userViewLibrary();
        else if (userAccountOption == "5") userSearchGame();
        else if (userAccountOption == "6") userSortLibrary();
        else if (userAccountOption == "7")
            cout << "\nUse Coupon While Buying.\n 'SAVE10' For 10 Percent Discount.\n 'SAVE20' For 20 Percent Discount.";
        else if (userAccountOption == "8") userViewHistory();
